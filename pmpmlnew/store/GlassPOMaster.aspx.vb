...
Namespace KDMT
    Partial Class GlassPOMaster
                    ...
                    If strIndentdetails.ToString <> "" Then
                        ...
                        For i = 0 To myarray.Length - 1
                            If myarray(i) <> "" Then
                                ...
                                cmd.Parameters.AddWithValue("@item_qty", mainarray(5))

                                cmd.Parameters.AddWithValue("@item_amt", (mainarray(5) * mainarray(4)))
                            ...
                            End If
                        Next
                        Response.Write("<script>alert('Saved Successfuly');if(confirm('Do You Want To Print PO')){window.location.href = '../report/rptGlassPOPrint.aspx?potype=" + PONoId + "&PoNo=" + txtPONo.Text + "&poyear=" + fin_year + "&category=" + ddlCategory + "&vendor=" + ddlvendor.Text + "' }</script>")
                    End If
                End If
            Else
        ...
        Protected Sub btn_print_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_print.Click
            Response.Write("<script>window.location.href = '../report/rptGlassPOPrint.aspx'</script>")
        End Sub
    End Class
End Namespace
